...
- 탐색기 다중 선택 `sZIP으로 압축` 및 `sZIP으로 압축 풀기`
...
GitHub Releases의 `sZIP_Setup_1.3.0.exe`가 권장 설치본입니다. 사용자 계정의 `%LOCALAPPDATA%\Programs\sZIP`에 설치하므로 관리자 권한이 필요하지 않습니다. `sZIP-1.3.0-net48.zip`은 설치하지 않고 사용할 수 있는 포터블 배포본입니다.

설치 프로그램은 선택에 따라 바탕 화면 바로가기, Windows 자동 시작, 탐색기 메뉴와 압축 확장자 연결을 등록합니다.
...
로컬 설치 프로그램 생성에는 Inno Setup 6가 필요합니다.
